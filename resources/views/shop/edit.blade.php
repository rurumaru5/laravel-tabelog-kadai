...
<div class="container mt-3" style="max-width: 720px;">
  ...
  <form action="{{ route('shop.update', [$shop->id]) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group" style="margin-bottom: 30px">
      ...
      <textarea class="form-control @error('description') is-invalid @enderror" id="textarea" rows="5" name="description">{{ $shop->description }}</textarea>
      @error('description')
      <p class="text-danger">{{ $message }}</p>
      @enderror
    </div>
    ...
    <div class="form-group" style="margin-bottom: 30px">
      ...
      <input type="text" class="form-control @error('low_price') is-invalid @enderror" id="low_price" name="low_price" value="{{ $shop->low_price }}" />
    ...
    <div class="form-group" style="margin-bottom: 30px">
      ...
      <input type="text" class="form-control @error('high_price') is-invalid @enderror" id="high_price" name="high_price" value="{{ $shop->high_price }}" />
      <small class="form-text text-muted">半角数字で入力してください。</small>
      @error('high_price')
      <p class="text-danger">{{ $message }}</p>
      @enderror
